...
def show_image2(depth_map, title):
    plt.axis('off')
    plt.imshow(depth_map, cmap='gray')
    # 保存为PNG文件
...
def paint_scatter(x, y, z):
    num_points = len(x)
    sample_size = max(1, num_points // 20)  # 至少选择1个点
    indices = np.random.choice(num_points, sample_size, replace=False)
    plt.figure(figsize=(16, 10.66))
    # 创建散点图
    plt.scatter(-x[indices], -y[indices], c=z[indices], cmap='viridis', s=0.01)  # s是点的大小
...
def process(lidar_path, mono_path, result_path):
    ...
    show_image2(lidar_depth, 'lidar_depth')
    show_image2(mono_depth, 'mono_depth')
    ...
    paint_scatter(v_coords, u_coords, z_values)
...
if __name__ == '__main__':
    # 文件夹路径
    mono_folder = '../data/Bispo_g5_select/depth'
    lidar_folder = '../data/Bispo_g5_select/lidar_depth'
    ...
    for file1, file2 in associated_images:
        ...
        result_path = '../data/Bispo_g5_select/fusion/' + file1
